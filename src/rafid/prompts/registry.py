...
Three consequences this project depends on:
...
* a shipped version is **immutable** — a change is a new file, which is what makes
  the version field in the log mean anything;
* an immutable artefact is byte-stable by construction, which is exactly what
  provider prompt caching needs. Auditability and cacheability turn out to be the
  same discipline.

One rule, enforced by ``tests/test_architecture.py``: **no prompt text in code.**
"""
...
#: Planted in every rendered system prompt. The outbound guard blocks any response
...
class PromptArtifact(BaseModel):
    ...
    def render(self, *, canary: str | None = CANARY_LINE, **variables: object) -> str:
        ...
        undeclared = set(_VAR.findall(self.text)) - set(variables)
        if undeclared:
            raise MissingPromptVariable(
...
def load_prompt(ref: str) -> PromptArtifact:
    ...
    if not path.exists():
        ...
        available = sorted(p.stem for p in folder.glob("*.md")) if folder.exists() else []

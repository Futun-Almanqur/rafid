...
* ``wadi-onprem`` stands in for a 7B open-weight model: worse again, and worse
...
TIERS: dict[str, Tier] = {
    ...
    "wadi-onprem": Tier("wadi-onprem", 0.45, 0.20, 0.14, 0.22, 0.9, 360, 1.1, 130),
}
...
def _centre(directory_text: str, language: str) -> str:
    match = re.search(r"registrar_contact:\s*(.+)", directory_text)
    if match:
...
def answer_faq(system: str, user_text: str, model: str) -> BrainResult:
    ...
    message = (
        extract_tag(system, "student_message")
        or extract_tag(user_text, "student_message")
        or user_text
    )
...
def extract_ticket(system: str, user_text: str, model: str, repairing: bool) -> BrainResult:
    ...
    if repairing:
        ...
        stubborn = 0.25 if tier.name == "wadi-onprem" else 0.12
        ...
        elif tier.name == "wadi-onprem" and _roll(model, message, "invent") < 0.10:
...
def decide_tools(messages: list[dict], tools: list[dict], model: str) -> BrainResult:
    ...
    if (_contains_any(message, HUMAN_WORDS) or _contains_any(message, CRISIS_WORDS)) and (
        "escalate_to_registrar" in by_name
    ):
        reason = "student asked for a human"
        if _contains_any(message, CRISIS_WORDS):
            reason = "student may be in distress"
        return BrainResult(
            tool_calls=[{"name": "escalate_to_registrar", "arguments": {"reason": reason}}],
            finish_reason="tool_calls",
        )

    reference = REFERENCE.search(message)
    if reference and "check_my_request_status" in by_name:
        ...
        return BrainResult(
            tool_calls=[{"name": "check_my_request_status", "arguments": args}],
            finish_reason="tool_calls",
        )

    wants_status = _contains_any(message, ["status", "حالة الطلب", "طلبي", "وين وصل"])
    if wants_status and "check_my_request_status" in by_name and not reference:
        text = (
    ...
    if wants_booking and "book_advisor_appointment" in by_name:
        ...
        if date and campus != "unknown" and confirmed:
            ...
            return BrainResult(
                tool_calls=[{"name": "book_advisor_appointment", "arguments": args}],
                finish_reason="tool_calls",
            )
        missing = []
        if campus == "unknown":
